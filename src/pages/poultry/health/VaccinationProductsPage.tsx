...
import { Pill, Plus, Search, Edit, Trash2, ChevronDown, ChevronUp, DollarSign, Package, AlertCircle, BarChart3, Settings, Clock, AlertTriangle, Eye } from "lucide-react"
import type { VaccineData, VaccineProduct, AdministrationMethod, PoultryVaccineInventory } from "@/lib/types"
...
interface NewVaccineProductForm {
...
const getBadgeColor = (id: number) => MEDICATION_BADGE_COLORS[Math.abs(Number(id)) % MEDICATION_BADGE_COLORS.length]
...
const expiryColorClass = (dateStr?: string | null): string => {
  ...
  if (m <= 0) return "bg-red-100 text-red-800 border-red-200"
  if (m === 1) return "bg-orange-100 text-orange-800 border-orange-200"
  if (m === 2) return "bg-amber-100 text-amber-800 border-amber-200"
  if (m === 3) return "bg-yellow-100 text-yellow-800 border-yellow-200"
  if (m === 4) return "bg-lime-100 text-lime-800 border-lime-200"
  return "bg-green-100 text-green-800 border-green-200"
}

const getProductInventories = (product: VaccineProduct): PoultryVaccineInventory[] => {
  const p: any = product as any
  const raw = p.inventory ?? p.inventories ?? p.poultry_vaccine_inventories ?? []
  const arr = Array.isArray(raw) ? raw : []
  return arr.map((inv: any) => ({
    ...inv,
    id: Number(inv.id),
    poultry_vaccine_product_id: Number(inv.poultry_vaccine_product_id ?? product.id),
  ...
  })) as PoultryVaccineInventory[]
}

function CreateVaccineProductModal({
  vaccines,
  isOpen,
  onClose,
  onSubmit,
  ...
  isOpen: boolean
  onClose: () => void
  onSubmit: (product: NewVaccineProductForm) => Promise<boolean>
}) {
  const defaultForm: NewVaccineProductForm = {
  ...
  const [formData, setFormData] = useState<NewVaccineProductForm>(defaultForm)
  ...
  useEffect(() => {
    const loadMethods = async () => {
      ...
      try {
        // Use global endpoint with farm_id to satisfy middleware
        const resp = await fetch(`/api/administration-methods?farm_id=${farm.id}`, {
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.manufacturer || !formData.poultry_vaccine_id || !formData.administration_method_id) {
      toast.error("Please fill in name, manufacturer, vaccine and administration method")
      return
    }
      ...
      <DialogContent className="max-w-4xl max-h-[95vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Add New Vaccine Product
          </DialogTitle>
          <DialogDescription>Create a new vaccine product</DialogDescription>
        </DialogHeader>
          ...
          <Card className="p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input id="name" value={formData.name} onChange={(e) => setFormData((p) => ({ ...p, name: e.target.value }))} placeholder="Enter product name" required />
              </div>
              <div>
                <Label htmlFor="manufacturer">Manufacturer *</Label>
                <Input id="manufacturer" value={formData.manufacturer} onChange={(e) => setFormData((p) => ({ ...p, manufacturer: e.target.value }))} placeholder="Enter manufacturer name" required />
              </div>
              <div>
                <Label htmlFor="vaccine">Vaccine *</Label>
                <Select value={formData.poultry_vaccine_id == null ? undefined : String(formData.poultry_vaccine_id)} onValueChange={(value: string) => setFormData((p) => ({ ...p, poultry_vaccine_id: Number(value) }))}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                <Select value={formData.administration_method_id == null ? undefined : String(formData.administration_method_id)} onValueChange={(value: string) => setFormData((p) => ({ ...p, administration_method_id: Number(value) }))}>
                  <SelectTrigger>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              <div>
                ...
                <Input id="dosage" type="number" step="0.01" value={formData.dosage ?? ''} onChange={(e) => setFormData((p) => ({ ...p, dosage: e.target.value === '' ? null : Number.parseFloat(e.target.value) }))} placeholder="e.g., 10" />
              </div>
              <div>
                <Label htmlFor="dosage_unit">Dosage Unit</Label>
                <Select value={formData.dosage_unit} onValueChange={(value) => setFormData((p) => ({ ...p, dosage_unit: value }))}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                <Input id="withdrawal" type="number" value={formData.withdrawal_period} onChange={(e) => setFormData((p) => ({ ...p, withdrawal_period: Number.parseInt(e.target.value) || 0 }))} min="0" />
              </div>
              <div>
                <Label htmlFor="withdrawal_unit">Withdrawal Period Unit</Label>
                <Select value={formData.withdrawal_period_unit} onValueChange={(value: "days" | "hours") => setFormData((p) => ({ ...p, withdrawal_period_unit: value }))}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                <Input id="min_stock_level" type="number" value={formData.min_stock_level} onChange={(e) => setFormData((p) => ({ ...p, min_stock_level: Number.parseInt(e.target.value) || 0 }))} min="0" />
              </div>
              <div className="md:col-span-2">
                <Label htmlFor="image_url">Image URL (optional)</Label>
                <Input id="image_url" value={formData.image_url || ''} onChange={(e) => setFormData((p) => ({ ...p, image_url: e.target.value }))} placeholder="https://..." />
              </div>
            </div>
          </Card>
          <DialogFooter>
            <Button type="button" variant="outline" onClick={handleClose}>Cancel</Button>
            <Button type="submit" className="bg-blue-600 hover:bg-blue-700">Add Vaccine Product</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
...
function ProductCard({
  ...
  vaccine: VaccineData
  onEdit: (product: VaccineProduct) => void
  ...
  if (invPage > totalInvPages) setInvPage(totalInvPages)
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 flex-1">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <Badge className={getBadgeColor(vaccine.id)}>{vaccine.name}</Badge>
                  </div>
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  ...
                  <Badge className={stockStatus}>{totalAvailable} units{isLowStock && " ⚠️"}</Badge>
                </div>
                <div className="text-right">
          ...
          <CardContent className="pt-0 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="font-medium">{(product as any).dosage ?? '-'}</p>
                </div>
              </div>
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="font-medium">{(product as any).withdrawal_period ?? 0} {(product as any).withdrawal_period_unit ?? ''}</p>
                </div>
              </div>
              <div className="flex items-start gap-2">
              ...
              {inventories.length > 0 ? (
                <>
                      ...
                      {pagedInventories.map((inv) => {
                        ...
                        return (
                          <TableRow key={inv.id}>
                            ...
                            <TableCell>{fmtDate(inv.manufacture_date)}</TableCell>
                            <TableCell>
                  ...
                  <div className="flex items-center justify-end gap-2">
                    <Button variant="outline" size="sm" disabled={invPage <= 1} onClick={() => setInvPage((p) => Math.max(1, p - 1))}>Previous</Button>
                    <span className="text-xs text-gray-600">Page {invPage} of {totalInvPages}</span>
                    <Button variant="outline" size="sm" disabled={invPage >= totalInvPages} onClick={() => setInvPage((p) => Math.min(totalInvPages, p + 1))}>Next</Button>
                  </div>
                </>
              ) : (
            ...
            <div className="flex justify-between items-center pt-4 border-t">
              ...
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={() => onEdit(product)} className="text-blue-600 hover:text-blue-700">
                  <Edit className="h-4 w-4 mr-1" />
                  Edit
                </Button>
                <Button size="sm" variant="outline" onClick={() => onDelete(product.id)} className="text-red-600 hover:text-red-700">
...
const VaccinationProductsPage: React.FC = () => {
  ...
  const loaderData = useLoaderData() as { vaccines: VaccineData[] | null } | VaccineData[] | null
  const vaccines: VaccineData[] = Array.isArray(loaderData)
    ? (loaderData as VaccineData[])
    : (loaderData?.vaccines ?? [])

  const products = useMemo(() => {
    return (vaccines ?? []).flatMap((v) => (v.products || []).map((p) => ({ product: p, vaccine: v })))
  }, [vaccines])

  const categoryOptions = useMemo(() => {
    const base: { value: string; label: string }[] = [{ value: 'all', label: 'All Vaccines' }]
    const vOpts = (vaccines ?? []).map((v) => ({ value: String(v.id), label: v.name }))
    return base.concat(vOpts)
  }, [vaccines])

  const filteredProducts = useMemo(() => {
    let list = products
    if (categoryFilter !== 'all') {
      list = list.filter((item) => String(item.vaccine.id) === categoryFilter)
    }
    if (searchTerm.trim()) {
      const term = searchTerm.toLowerCase()
      list = list.filter(({ product, vaccine }) =>
  ...
  const handleCreateVaccineProduct = async (form: NewVaccineProductForm): Promise<boolean> => {
    try {
      const token = GetToken()
      const farm = getFarm()
      if (!token || !farm) {
        toast.error("Missing auth or farm context")
        return false
      }
      const payload = {
        poultry_vaccine_id: form.poultry_vaccine_id as number,
        name: form.name.trim(),
        manufacturer: form.manufacturer.trim(),
        administration_method_id: form.administration_method_id as number,
        withdrawal_period: form.withdrawal_period || undefined,
        withdrawal_period_unit: form.withdrawal_period_unit || undefined,
      ...
      if (res.success) {
        ...
        const msg = Array.isArray(res.error) ? res.error.join("\n") : (res as any).error || "Failed to create vaccine product"
        toast.error(msg)
        return false
      }
    } catch (e) {
  ...
  const totalProducts = (vaccines ?? []).reduce((count, v) => count + ((v.products || []).length), 0)
  const lowStockCount = (products ?? []).reduce((sum, { product }) => {
    const available = getProductInventories(product).reduce((s, inv) => s + (Number(inv.available_quantity ?? inv.quantity) || 0), 0)
    const minStock = Number((product as any).min_stock_level ?? 0)
    return sum + (available <= minStock ? 1 : 0)
  }, 0)
  const totalStockValue = (vaccines ?? []).reduce((sum, v) => {
    const productValue = (v.products || []).reduce((pSum, prod) => {
  ...
  const categoriesCount = new Set((vaccines ?? []).map((m) => m.type || "unknown")).size
  ...
  return (
    <TooltipProvider>
      <div className="min-h-screen  p-4 md:p-6">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold text-gray-900 mb-2">Vaccination Products</h1>
                <p className="text-gray-600">Manage your poultry vaccine inventory and safety information</p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <Card className="bg-gradient-to-br from-purple-500 to-purple-600 text-white border-0">
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-purple-100 text-sm">Vaccines</p>
            ...
            <CardContent className="p-6">
              <div className="space-y-4">
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="flex-1">
                    <Label htmlFor="search" className="text-sm font-medium text-gray-700 mb-2 block">Search Vaccines</Label>
                    <div className="relative">
                      <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
                      <Input id="search" placeholder="Search by product or vaccine..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 border-gray-200" />
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                  <div>
                    <Label className="text-sm font-medium text-gray-700 mb-2 block">Vaccine</Label>
          ...
          <div className="space-y-4">
            {filteredProducts.length === 0 ? (
              ...
              filteredProducts.map(({ product, vaccine }) => (
                <ProductCard key={`${vaccine.id}-${product.id}`} product={product} vaccine={vaccine} onEdit={() => {}} onDelete={() => {}} onViewDetails={() => {}} />
              ))
            )}
          </div>
        </div>
      ...
      <CreateVaccineProductModal vaccines={vaccines} isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} onSubmit={handleCreateVaccineProduct} />
      <ToastContainer position="top-right" autoClose={3000} newestOnTop closeOnClick pauseOnHover draggable theme="colored" />
    </TooltipProvider>
  )
}

export default VaccinationProductsPage
